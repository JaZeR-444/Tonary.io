...
- **2026-07-02 — Candidates STAGED (not shipped): Harmor, Maximus, Gross Beat.** Re-keyed from DEEPER DIVES into schema-shaped JSON at [`migration-staging/`](migration-staging/): 3 Plugin Records, 7 Sound Design Notes, 6 Source References (presets deferred per content-policy §3; Gross Beat's factory-preset map explicitly excluded). Rephrased-not-reproduced, all cited to the official manual, every candidate passes the real `SeedValidator` + no-id-collision check (`test/data/staging_candidates_test.dart`) and the gate #4 legal-content checklist ([`migration-staging/README.md`](migration-staging/README.md)). **Held out of `assets/seed/` pending OQ-12 gate #5 sign-off.** Proves the post-legal migration path end-to-end without publishing.
