...
- **2026-07-02 — Candidates STAGED (not shipped), 27 total:** Harmor, Maximus, Gross Beat, Slicex, Vocodex, Patcher, Newtone, Harmless, Fruity Limiter, Toxic Biohazard, Sawer, Morphine, FPC, Fruity Convolver, Fruity Delay 3, PoiZone, Fruity Love Philter, Fruity Reeverb 2, Soundgoodizer, Fruity Compressor, Fruity Filter, Fruity DX10, Fruity Granulizer, Fruity Slicer, Fruity Vocoder, Fruity Blood Overdrive, Fruity WaveShaper. Re-keyed from DEEPER DIVES into schema-shaped JSON at [`migration-staging/`](migration-staging/): **27 Plugin Records, 46 Sound Design Notes, 54 Source References** (presets deferred per content-policy §3; Gross Beat's factory-preset map explicitly excluded; Edison / DirectWave / Transient Processor deferred — template data or unverified tier). Rephrased-not-reproduced, all cited to the official manual, every candidate passes the real `SeedValidator` + no-id-collision check (`test/data/staging_candidates_test.dart`) and the gate #4 legal-content checklist ([`migration-staging/README.md`](migration-staging/README.md)). Tier mix: Free (Fruity Limiter, FPC, Fruity Convolver, Fruity Delay 3) vs Premium (the bundle synths/editors).
- **2026-07-02 — PROMOTED to seed (seedVersion 3). ✅ SHIPPED.** Owner authorized the OQ-12 gate #5 release; all 27 staged candidates merged into `assets/seed/` (now **30 plugins, 51 sound-design notes, 60 source references** total incl. the pilot). `kSeedVersion` bumped 2→3; staging apparatus removed. Full seed re-validated by `SeedValidator` (bundled-dataset test) and the in-memory SQLite integration test; `flutter analyze` clean, suite green. Migration now continues by appending remaining plugins directly to the seed.
- **2026-07-02 — FULL LIBRARY SHIPPED (seedVersion 10). ✅ COMPLETE.** Owner directed migrating all plugins into the app. Batches 1–7 appended the remaining FL native/bundle plugins directly to `assets/seed/`, taking the library to **120 plugins** — every sound-design-relevant FL Studio stock/bundle plugin. Each record: re-authored from documented function, cited to the official manual, no fabricated values, validated per batch by the bundled-dataset `SeedValidator`. **Intentionally excluded** (see `assets/seed/README.md`): mobile apps, non-audio visual/haptic plugins (Fruity Dance/Vibrator/Video Player, Razer Chroma, ZGameEditor Visualizer), host/interop (Plugin Wrapper, ReWire, Transporter), and a few with no resolvable manual page. FL plugin roster migration complete.
